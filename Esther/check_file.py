...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", "--type", type=str, default="交易规模", help="数据类型,交易规模数据,还是收入数据")
    args = parser.parse_args()
    file = args.file
    type_ = args.type
    output_dir = args.output_dir
    columns = CONFIG["scale"]["columns"]
    target_column = CONFIG["scale"]["target_columns"]
    date_column = CONFIG["scale"]["date_column"]
    date_format = CONFIG["scale"]["date_format"]
    try:
