...
def getBrand(symbol, company_name):
    ...
    for data in datas :
        ...
        pdata.append(data['total_assets'][0])
        print(item_info)
    return pdata;
...
def process_item():
    ...
    try:
        ...
        query_sql = "select  ID,  HEBING,       DATA_ID,       COMPANY_NAME,       STOCK_CODE, " \
                    ...
                    " from " + TABLE_NAME + "  where COMPANY_NAME is not null and STOCK_CODE  is null "
        ...
        for i in cue.fetchall():
            ...
            if pdata == None:
                continue
            as_update_sql = "update " + TABLE_NAME + "  set total_assets13=%s " \
                                             ", STOCK_CODE=%s" \
                                              ", DESCRIPTION=%s " \
                                             " where ID=%s"
            pdta1 = ''
            ...
            if len(pdata) < 6:
                ...
                pdta2 = pdata[5]
            data = (pdta1,sc,pdata[0],i[0])

            try:
                cue.execute(as_update_sql, data)
                print(data)
