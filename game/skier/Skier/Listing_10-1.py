...
def create_map():
    ...
    for i in range(10):                 # 10 obstacles per screen 
        ...
        if not (location in locations):        # prevent 2 obstacles in the same place
            ...
            if type == "tree": img = "D:/tmp/data/image/skier_tree.png"
            elif type == "flag":  img = "D:/tmp/data/image/skier_flag.png"
...
while running:
    ...
    if hit:
        if hit[0].type == "tree" and not hit[0].passed:  #crashed into tree  
            points = points - 100
            skier.image = pygame.image.load("D:/tmp/data/image/skier_crash.png")  # crash image
            animate()  
            pygame.time.delay(1000)
            skier.image = pygame.image.load("D:/tmp/data/image/skier_down.png")  # resume skiing
            skier.angle = 0
            speed = [0, 6]
            hit[0].passed = True
